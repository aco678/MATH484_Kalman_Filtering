...
%Solve ODE
IC = [101; 1; 0];           %Actual initial conditions
mT_step = 1;                %Size of step between measurements
measT = [0:mT_step:90];    %Timing of all measurements
...
actY = act(:,3);
TF = isoutlier(actY)
intY = rmoutliers(actY); %remove outliers
intY(72) = [];
...
%Variance in Data
dataSmoo = smoothdata(intY);
dataVar = std(intY-dataSmoo).^2;

%Variance with SSA
TS = SSA(intY);
TS = TS{1}+TS{2}; %Retain 2 eigenvalues for a fairly smooth figure
dataVarTS = std(intY-TS).^2;
...
store_xTraj = NaN(91*(nM-1),3*nE); %Each ensemble will have a single matrix of time points - needs two levels?
...
for k = 1:nM-1
    ...
    timeGap = linspace(time_start, time_end, 91);
    ...
    for i = 1:nE
        ...
        store_xTraj(91*(k-1)+1:91*k,3*(i-1)+1:3*i) = Y;
    end
...
plot(trajTime,EnsTrajS,'b-','Color', [0,0,1,0.3]);
xlabel('Time (Days)');
ylabel('Individuals');
title('Susceptible');
...
plot(trajTime,EnsTrajR,'b-','Color', [0,0,1,0.3]);
xlabel('Time (Days)');
ylabel('Individuals');
title('Recovered');
...
plot(trajTime,EnsTrajI,'b-','Color', [0,0,1,0.3]);
xlabel('Time (Days)');
ylabel('Individuals');
title('Infected');
...
if Video
    ...
    for k = 1:15
        ...
        plot(trajTime(1:trajSt),EnsTrajI(1:trajSt,:),'b-','Color', [0,0,1,0.3]);
        xlabel('Time (Days)');
        ylabel('Individuals');
        ...
        plot(trajTime(1:trajSt),EnsTrajR(1:trajSt,:),'b-','Color', [0,0,1,0.3]);
        xlabel('Time (Days)');
        ylabel('Individuals');
    ...
    for k = 1:trajSS-1
        ...
        plot(trajTime(1:trajES),EnsTrajI(1:trajES,:),'b-','Color', [0,0,1,0.3]);
        xlabel('Time (Days)');
        ylabel('Individuals');
        title('Observed State: Infected');
        xlim([0,30]);
        ...
        plot(trajTime(1:trajES),EnsTrajR(1:trajES,:),'b-','Color', [0,0,1,0.3]);
        xlabel('Time (Days)');
        ylabel('Individuals');
        title('Unobserved State: Suceptible');
        xlim([0,30]);
    ...
    for k = 1:15
        ...
        plot(trajTime(1:trajEn),EnsTrajI(1:trajEn,:),'b-','Color', [0,0,1,0.3]);
        xlabel('Time (Days)');
        ylabel('Individuals');
        title('Observed State: Infected');
        xlim([0,30]);
        ...
        plot(trajTime(1:trajEn),EnsTrajR(1:trajEn,:),'b-','Color', [0,0,1,0.3]);
        xlabel('Time (Days)');
        ylabel('Individuals');
        title('Unobserved State: Suceptible');
        xlim([0,30]);
